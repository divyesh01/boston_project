...
-- Staged-transaction cleanup (abort, expiry sweep, commit conflict rollback)
-- deletes guard rows by (account_id, request_hash), which the primary key on
-- (account_id, mutation_id) cannot serve. Guard rows written by `mutate` are
-- retained deliberately as the replay-idempotency record, so this table grows
-- with mutation volume and an unindexed delete would degrade over time.
CREATE INDEX idx_business_mutation_guard_request
  ON business_mutation_guard (account_id, request_hash);
...
CREATE TABLE business_staging_transaction (
  ...
  FOREIGN KEY (account_id, base_generation_id)
    REFERENCES business_dataset(account_id, generation_id),
  FOREIGN KEY (account_id, staging_generation_id)
    REFERENCES business_dataset(account_id, generation_id) ON DELETE CASCADE
);
CREATE INDEX idx_business_staging_transaction_lookup
  ON business_staging_transaction (account_id, status, expires_at);
...
CREATE TABLE business_staging_target (
  ...
  FOREIGN KEY (account_id, tx_id)
    REFERENCES business_staging_transaction(account_id, tx_id) ON DELETE CASCADE
);
...
-- ---------------------------------------------------------------------------
-- BULK IMPORT DATA PLANE (R2 Bundle Manifest Control Plane)
-- ---------------------------------------------------------------------------
