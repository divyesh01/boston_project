...
const archMatch = content.match(/```mermaid[\s\S]*?```/);
const archDiagram = archMatch ? archMatch[0] : '';
...
const trouble = getSec(/14\. THE 9 KNOWN PROBLEMS/i) + '\n\n' + getSec(/19\. EMERGENCY PLAYBOOK/i);
fs.writeFileSync('docs/brain/BRAIN_TROUBLESHOOTING.md', trouble);

const index = getSec(/20\. APPENDIX/i);
fs.writeFileSync('docs/brain/BRAIN_INDEX.md', index);
...
| 🗺️ **Index** | \`docs/brain/BRAIN_INDEX.md\` | Verify file paths (418-file complete catalog). |
