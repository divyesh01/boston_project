...
- A caller who legitimately holds a grant on a property whose id is literally the staging sentinel — an id no real property can have, since real ids are `prop_` plus 32 hex characters — is *denied* on a global target rather than granted. A fail-closed collision, recorded so a later reader does not misread it as an escalation.
