...
The search script lives inside this skill's own directory. The paths below are relative to the **repository root** (this is a project-scoped install, so `${CLAUDE_PLUGIN_ROOT}` is not set). Run from the repo root, or substitute an absolute path:
