...
graph TD
    ...
    subgraph Cloud [Cloudflare Worker]
        API[Same-origin /api authentication]
        DB_Auth[(Production D1<br/>users, sessions, MFA challenges)]
        API <--> DB_Auth
    end
...
| Area | Status |
|------|--------|
| Production authentication (2026-09-01) | **LIVE and verified.** Worker `6b04c93d-f5e4-4de7-9832-357a8aeffee5` serves same-origin server authentication and fixes the authenticated blank-page incident by deriving owner/admin frontend route capabilities from the authoritative server role while preserving stored overrides. The production owner row remains unchanged with role `owner`, property scope `all`, and its versioned credential; no D1 business-data route was enabled. The earlier production smoke harness passed 22/22, and the post-fix remote runtime probe passed 8/8. Immediate rollback target: `623b3ee9-6815-43fb-a70c-1c1cdea0a2c8`. Hotel/business rows remain in each browser's IndexedDB because both `VITE_USE_D1_API` and `ENABLE_D1_DATA_API` are false. |
