...
// 5. Update auth endpoints for server-side fallbacks
const originalAuthLogoutStr = `    try {
...
code = code.replace(
  ...
  `    if (functionName === 'deleteAccount' || functionName === 'audit_clear' || functionName.startsWith('auth_')) {
