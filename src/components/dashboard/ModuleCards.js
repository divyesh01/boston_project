...
function ModuleCard({ theme, statLabel, statValue, statSub }) {
    ...
    const Icon = t.icon;
    return (_jsxs(Link, { to: t.to, className: "fx-clickable group relative block overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0F1F35]/90 p-5 transition-all duration-300 hover:-translate-y-1", style: { boxShadow: rgba("#000000", 0.45) }, onMouseEnter: (e) => {
        ...
        }, children: [_jsx("div", { className: "pointer-events-none absolute -top-16 -right-16 h-40 w-40 rounded-full opacity-40 blur-2xl transition-opacity duration-300 group-hover:opacity-100", style: { background: `radial-gradient(circle, ${rgba(t.text, 0.18)}, transparent 70%)` } }), _jsx("div", { className: "pointer-events-none absolute inset-x-0 top-0 h-[2px]", style: { background: `linear-gradient(90deg, transparent, ${t.text}, transparent)` } }), _jsxs("div", { className: "relative flex items-center gap-3", children: [_jsx("span", { className: "flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border", style: { borderColor: rgba(t.text, 0.4), background: rgba(t.text, 0.1), color: t.text }, children: _jsx(Icon, { className: "h-5 w-5" }) }), _jsxs("div", { children: [_jsx("p", { className: "text-[11px] font-semibold uppercase tracking-[0.2em]", style: { color: t.text }, children: t.title }), _jsxs("h3", { className: "mt-0.5 font-heading text-lg font-semibold text-white", children: [t.title, " Module"] })] })] }), _jsxs("div", { className: "relative mt-5", children: [_jsx("p", { className: "text-[10px] uppercase tracking-widest text-slate-500", children: statLabel }), _jsx("p", { className: "mt-1 font-heading text-3xl font-semibold tabular-nums text-white", children: statValue }), statSub && _jsx("p", { className: "mt-1 text-xs text-slate-400", children: statSub })] }), _jsx("p", { className: "relative mt-2 text-xs leading-relaxed text-slate-400", children: t.description }), _jsxs("p", { className: "relative mt-4 flex items-center gap-1.5 text-xs font-medium opacity-80 transition-opacity group-hover:opacity-100", style: { color: t.text }, children: [t.action, _jsx(ArrowRight, { className: "h-3.5 w-3.5 transition-transform duration-300 group-hover:translate-x-1" })] })] }));
}
// 6-digit hex → rgba
...
export default function ModuleCards() {
    ...
    const payroll = useMemo(() => allPayroll.filter((p) => inRange(p.pay_period_start, dateRange.from, dateRange.to)), [allPayroll, dateRange]);
    const stats = useMemo(() => {
    ...
    return (_jsxs("section", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("p", { className: "text-[11px] uppercase tracking-[0.3em] text-slate-500", children: "Command Center" }), _jsx("span", { className: "text-[10px] text-slate-600", children: "Tap a module to jump in" })] }), _jsxs("div", { className: "grid gap-4 sm:grid-cols-2 xl:grid-cols-3", children: [_jsx(ModuleCard, { theme: "emerald", ...stats.payroll }), _jsx(ModuleCard, { theme: "cyan", ...stats.rooms }), _jsx(ModuleCard, { theme: "gold", ...stats.revenue })] })] }));
}
