...
# 18. ACCOUNT-GLOBAL BUSINESS RECORDS (2026-09-03)

Before this change, an entity row carrying no property association could not
reach the server at all: activation refused any generation containing a property
key it could not map, and `""` types to `s:0:`, which is never present in
`business_property_map`. The client emits that encoding for legitimately
property-less rows — historical payroll, an expense booked while the property
filter is "all" — so those rows were the reason a real dataset could not
activate.

Such rows are now first-class **account-global** records: `property_key` is
exactly `s:0:` (`GLOBAL_PROPERTY_KEY`, recognised by `isGlobalPropertyKey`) and
`server_property_id` is SQL `NULL`. Only a `scope.all` caller may read, create,
update, or delete one.

The contract, stated as invariants rather than as a diff:

- **Read.** `scopedRecordClause` emits `server_property_id IN (...)` for a restricted caller, and SQL `NULL` never matches `IN`, so a global row is invisible without `scope.all`. It is not hidden by an extra predicate that could be forgotten — it is excluded by the same clause that already enforces every other property boundary.
- **Write, single record.** `mutate` decides the global case inside the same hoisted authorization block as the roster and mapped-property cases, above the record load, so a restricted caller is refused before any existence-revealing conflict check runs.
- **Write, batch.** `uploadTransactionChunk` refuses a global operation from a restricted caller during its first pass over the operations, before any mapping lookup, and deliberately keeps `s:0:` out of the `property_key IN (...)` list — there is no map row to find, because `business_property_map.server_property_id` is `NOT NULL` with a foreign key to the property roster.

- **Re-homing.** The single-record path loads the stored row and compares **both** its `server_property_id` and its `property_key` against the request before the `ON CONFLICT ... DO UPDATE SET server_property_id=excluded.server_property_id` runs. The key half is not redundant: a `business_record` row whose stored `property_key` has drifted from its `server_property_id` would otherwise be silently re-keyed, and a delete carrying no `property_key` would match a stored global row. The batch path reaches the same guarantee by a different mechanism, and the difference is worth knowing — it loads no row in JS, so its present-row guard compares `row_hash` and `server_property_id` only, never `property_key`, which the upsert then overwrites from `excluded`. Cross-property movement is still impossible there, because reaching that upsert requires the guard to have already matched the stored `server_property_id`, so the batch path can only re-key a row already sitting on the submitted property. The asymmetry is real and deliberate: on a drifted row the single-record path answers 403 where the batch path silently repairs the key.
- **Commit.** `commitTransaction`'s SQL scope guard stages a global target under the sentinel `GLOBAL_STAGING_TARGET_ID`, which is excluded from the grant arm of the predicate, leaving only the two unrestricted arms — role `owner`/`admin`/`gm`, or `property_access_mode='all'` — which is exactly `scope.all`. `business_staging_target.server_property_id` is `NOT NULL`, so the sentinel exists because a global target cannot be represented as NULL there.
- **Activation.** The orphan check skips `s:0:` instead of demanding a mapping, and no `business_property_map` row is ever written for it. That holds by *reservation*, not by omission, at three points: the migration chunk upload refuses a `Property` row keyed `s:0:`; the single-record roster path refuses it inside the same hoisted authorization block as its `scope.all` gate, which places the refusal above the record load so it cannot double as an existence oracle and covers the upsert and delete arms with one clause; and activation refuses a roster carrying it after the mapping table is built and before the roster-write loop runs — it must come after, since it tests that table; there are two loops and only the second one writes — which is the only one of the three that protects a generation already staged on the server. Without the reservation a roster row whose `id` is the empty string would key on `s:0:`, be mapped to a real property, and activation's `UPDATE business_record SET server_property_id=? WHERE ... AND property_key=?` would re-home every account-global record onto it: `scope.all`-only rows silently downgraded to property-scoped rows a grantee can read, left in the drifted state the re-homing rule above then makes permanently unwritable. The reachable consequence is worse than disclosure — the roster delete arm would resolve the poisoned mapping onto that **real** property and delete it, cascading away its `user_property_access` grants and its `business_record` rows. The reservation is what stands between an in-band key collision and destruction of a live property. An empty or SQL `NULL` staged `property_key` remains an orphan and still fails activation with 422 — only the exact `s:0:` encoding is global.

The sentinel is **in-band**, and that is the one structural weakness of this
design. `typedRecordKey` encodes a string of length zero as `s:0:`, so `s:0:` is
simultaneously "no property" and "a property id that is the empty string" — the
encoder is not injective over the domain it is now asked to discriminate. An
out-of-band marker (`property_key: null`, or a `g:` prefix the encoder cannot
emit) would have reserved itself by construction. Because the sentinel instead
lives inside the encoder's own range, **every site that admits a caller-supplied
property id must reserve it**, and the reservation named under *Activation* above
is load-bearing rather than defensive. A future encoder change must keep it.

Recorded residuals — known, not silently patched:

- The existence differential recorded in section 17 now also reaches global rows: through `mutate`, a restricted caller can still distinguish "this `record_key` is taken by an account-global row" (403) from "this `record_key` is free" (200). Reads, writes, creates and deletes are all closed; only existence leaks. Closing it needs the same key-namespace migration. Section 17's `mutation_id` replay note extends the same way — that early-return still answers above the authorization block and is not property-scoped, so a restricted caller replaying a global record's `mutation_id` gets the row back. It is a confirmation oracle rather than a disclosure primitive: the 200 arm requires reproducing the exact canonical body, so the caller must already hold the row.
- A caller who legitimately holds a grant on a property whose id is literally the staging sentinel is *denied* on a global target rather than granted. No code path mints that id — `deterministicPropertyId` emits `prop_` plus 32 hex, and `worker/entities.js` emits a UUID or `imp_` plus 32 hex — and the commit guard refuses it even if a row carrying it already exists, which `scripts/probe-business-sync-global-records.mjs` proves in the check *"a caller downgraded mid-transaction cannot commit a global target even when granted the sentinel id"*: it inserts a real `property` row with that id, grants a `manager` an `all`-mode grant that includes it, asserts the staged target carries that exact id (or the grant proves nothing), downgrades the caller to `specific` mid-transaction, and asserts the commit returns 403 `auth_scope_revoked` — while a control commit by the **same** downgraded caller on its genuinely granted property returns 200, so the 403 cannot pass for the wrong reason. Do **not** restate this as "an id no real property can have": ids are not constrained to the `prop_` shape (`scripts/data/seed-staging-acceptance.sql` seeds `HOTEL_A`/`HOTEL_B`), so the guarantee is the guard, not the format. A fail-closed collision, recorded so a later reader does not misread it as an escalation.
- The present-row guard's "do not constrain on property" arm is reachable only with a non-NULL bound property id. Every call site was enumerated: the roster arms pass a non-NULL id with the flag defaulted off, the two non-roster arms pass the record's own global flag, and the chunk path rejects `entity = "Property"` outright — so the combination of that arm with a NULL bound id is unreachable by construction, not merely unobserved.
- Nothing restricts *which* entity may be account-global. An unrestricted caller can upload a global `Room`, which is conceptually meaningless though structurally harmless. The financial case is not harmless and is the reason to close this: a global `TransactionLine` or `Expense` would be visible to an owner and invisible to a granted manager, so the two would compute different cent-exact YTD totals with no reconciliation error raised anywhere. Production is clean today — grouping every staged `property_key` absent from the roster returns `s:0:` on `PayrollRun` and `Staff` only — so this is a latent hole, not live divergence. Narrowing the global set to `Staff` and `PayrollRun` is deferred, not done.

The authoritative regression is `scripts/probe-business-sync-global-records.mjs`
— 23 checks over the production DDL with `PRAGMA foreign_keys = ON`. The first
17 were proven failing-first and mutation-tested against nine independent
mutations of the shipped implementation, one per clause above, and killed **9 of
9**; every mutation after the first failed exactly one check, so no check is
passing for an unrelated reason. Five of those clauses had no killing test when
the implementation was first reviewed, and the four checks that close them exist
only because the mutation run exposed the gap — which is why a clause without a
killing test is recorded here as unproven regardless of review depth.

Adversarial review then found a tenth defect that all nine mutations had missed:
the in-band sentinel was never reserved. Mutation testing perturbs the
implementation that exists; it cannot surface a guard nobody wrote. The six
checks closing it were proven failing-first against the unreserved
implementation — 17 passed, 6 failed — and each pins an *effect* rather than
only a status code: no sentinel mapping minted, no account-global record
re-homed, the real roster row and its access grants intact, and a grantee still
