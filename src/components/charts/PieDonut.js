 ...
 * }}
 */
{ data = [], type = "donut", height = 320, outerRadius = 110, innerRadius, minShareLabel = 0.5, maxSlices = 12, formatter = money2, showLegend = true, colors, centerLabel, centerSub, }) {
    ...
    return (_jsxs("div", { style: { height }, className: "relative w-full", children: [_jsx(ResponsiveContainer, { width: "100%", height: "100%", children: _jsxs(PieChart, { children: [_jsx(Pie, { data: chart, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", outerRadius: outerRadius, innerRadius: radiusInner, paddingAngle: 2, label: renderLabel, labelLine: { stroke: "#475569", strokeWidth: 1 }, isAnimationActive: false, children: chart.map((entry, i) => (_jsx(Cell, { fill: entry.color || colorAt(i), stroke: "#040D1A", strokeWidth: 2 }, i))) }), _jsx(Tooltip, { contentStyle: tip, formatter: (value, name, entry) => {
                                ...
                                return (_jsxs("span", { style: { color: "#94a3b8" }, children: [value, " \u2014 ", formatter(entry?.payload?.value || 0), " (", share, "%)"] }));
                            } }))] }) }), centerLabel && (_jsxs("div", { className: "pointer-events-none absolute inset-x-0 top-1/2 -translate-y-1/2 text-center", children: [_jsx("p", { className: "font-heading text-2xl font-semibold text-white", children: centerLabel }), centerSub && _jsx("p", { className: "text-xs text-slate-500", children: centerSub })] }))] }));
}
