...
- [ ] **Two server-shaped modules sit under `src/` waiting to become a white screen.** `src/lib/corsConfig.js:19` and `src/lib/securityHeaders.js:20` both read `process.env.…` at **module scope**. `process` does not exist in the browser and Vite does not shim it, so importing either from anything reachable by `src/main.jsx` is an immediate `ReferenceError` on load — a blank page, not a degraded feature. Today **nothing imports them**, which is the only reason this is not already a live defect, and they are left in place rather than edited on that basis. `probe-deploy-config` §6 now fails if anything imports them or if a new file adopts the same shape. **OBSERVED**
...
*Originally generated by code review and targeted Node probes on 2026-08-15, then updated in place as the findings were fixed on 2026-08-16. Claims are labelled OBSERVED, INFERRED, or NOT RUN — please treat INFERRED items as requiring confirmation, and NOT RUN items as genuinely unknown rather than passing. Ticked items name the command that proves them.*
