...
function computeRangeFromMonths(year, months, latestDate) {
    ...
    if (year >= today.getFullYear() && maxM >= today.getMonth()) {
        ...
        if (to < iso(year, minM, 1))
            to = iso(year, maxM, lastDay(year, maxM));
    }
...
export function GlobalFiltersProvider({ children }) {
    ...
    const accessibleProperties = useMemo(() => properties.filter((p) => canAccessProperty(p.id)), [properties, canAccessProperty]);
    ...
    const isMonthSelected = useCallback((dateStr) => {
        if (!dateStr)
            return false;
        if (period !== "monthly")
            return true; // only filter when in multi-month mode
        if (months.length === 0)
            return true; // no months selected = all
