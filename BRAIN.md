...
| Area | Status |
|------|--------|
| Production authentication (2026-09-01) | **LIVE and verified.** Worker `7675fe25-fa8f-4c1d-860a-4f43689e156d` serves same-origin server authentication. Production D1 contains exactly one versioned owner credential and zero legacy credentials; `PASSWORD_PEPPER_V1` is a Worker secret. The production smoke harness passed 22/22: two independent sessions for the same owner, logout isolation, revocation, expiry, CSRF rejection, MFA single-use, lockout/recovery, invalid-session fallback blocking, and cleanup. Rollback target: `36993034-a938-4fb9-8b19-e7ac87820b34`. Hotel/business rows remain in each browser's IndexedDB because both `VITE_USE_D1_API` and `ENABLE_D1_DATA_API` are false. |
