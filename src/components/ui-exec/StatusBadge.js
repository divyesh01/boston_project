...
export default function StatusBadge({ status = "draft", size = "md", onClick = undefined }) {
    ...
    return (_jsxs("span", { onClick: onClick, className: `inline-flex select-none items-center rounded-full border font-medium tracking-wide ${dims} ${meta.bg} ${meta.border} ${meta.text} ${onClick ? "cursor-pointer" : "cursor-default"} ${meta.pulse ? "status-pulse" : ""}`, style: { boxShadow: meta.glow }, title: meta.label, children: [_jsx(Icon, { className: `${iconDims} ${meta.pulse ? "animate-pulse" : ""}` }), _jsx("span", { className: "hidden sm:inline", children: meta.label }), _jsx("span", { className: "sm:hidden", children: meta.emoji })] }));
}
