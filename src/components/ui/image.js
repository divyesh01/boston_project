...
const ImageWrapper = React.forwardRef(({ aspectRatio, className, style, children }, ref) => (_jsx("span", { ref: ref, className: cn("inline-block relative", className), style: { aspectRatio, ...style }, children: children })));
ImageWrapper.displayName = "ImageWrapper";
const ResponsiveImage = React.forwardRef(({ parsed, fittingType, focalPoint, quality, className, style, aspectRatio, onLoad, ...props }, parentRef) => {
    ...
    // first paint.
    return (_jsxs(ImageWrapper, { ref: wrapperRef, aspectRatio: aspectRatio, className: className, style: style, children: [options && !loaded && (_jsx("img", { src: buildTransformUrl(parsed, {
                    ...options,
                    width: 20,
                    ...
                    filter: "blur(10px)",
                    transform: "scale(1.1)",
                } })), options && (_jsx("img", { ref: imgRef, src: buildTransformUrl(parsed, options), srcSet: buildSrcSet(parsed, options), loading: "lazy", className: cn("w-full h-full inset-0 absolute", fittingType === "fit" ? "object-contain" : "object-cover"), onLoad: (e) => {
                    setLoaded(true);
                    onLoad?.(e);
                }, ...props }))] }));
});
 ...
 */
const Image = React.forwardRef(({ src, fittingType = "fill", originWidth, originHeight, focalPointX, focalPointY, quality = 90, ...props }, ref) => {
    ...
    if (!src) {
        ...
        return _jsx("img", { ref: ref, src: FALLBACK_IMAGE_URL, ...imageProps, "data-empty-image": true });
    }
    ...
    if (!parsed) {
        ...
        return (_jsx("img", { ref: ref, src: imgSrc, ...imageProps, "data-error-image": isErrorUrl || undefined }));
    }
    ...
    return (_jsx(ResponsiveImage, { ref: ref, parsed: parsed, fittingType: fittingType, focalPoint: focalPoint, quality: quality, aspectRatio: aspectRatio, ...imageProps }));
});
